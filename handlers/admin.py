...
async def admin_list_videos(message: Message):
    ...
    for video in videos:
        ...
        caption = (
            f"🎬 <b>{title}</b> ({video['position']}-o'rin)\n"
            f"{description}\n\n"
...
async def cb_delete_ask(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f"⚠️ Ushbu videoni o'chirishni tasdiqlaysizmi?\n\n🎬 <b>{escape(video['title'])}</b>",
...
async def export_excel(message: Message):
    ...
    results_sheet.append(["O'rin", "Ishtirokchi", "Ovozlar soni", "Foiz"])
    ...
    for i, video in enumerate(videos, start=1):
        ...
        results_sheet.append([i, video["title"], video["votes_count"], round(percent, 1)])
